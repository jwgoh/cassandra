...
.Warning
====
Creating new tables with the `COMPACT STORAGE` option is discouraged. Since Cassandra 3.0,
...
.Warning
====
`DROP COMPACT STORAGE` should be avoided for production environments. There are still cases where a change of
...
====
 ...
 compatibility for definitions created before CQL version 3 and should be avoided for new tables. 4.0 supports partially
...
.Warning
====
The `CLUSTERING ORDER BY` table option influences how queries can be done against the table. This option is immutable. That is, if it is applied to a table there is no way to remove or update the option. Careful consideration must be made when applying this option to a table.
====
